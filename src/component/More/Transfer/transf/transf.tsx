...
// const ConfirmTransfer = async () => { 
//   if (!userId || !receiverId || inputValue <= 0) {
//       console.warn("Transfer not initiated: Invalid user ID, receiver ID, or input value.");
//       return;
//   }
...
//           if (!senderDoc.exists() || !receiverDoc.exists()) {
//               console.error("Transaction failed: Invalid sender or receiver ID.");
//               throw new Error("Invalid sender or receiver ID.");
//           }
...
//           if (senderData.Okuse < inputValue) {
//               console.error(`Transaction failed: Insufficient Okuse balance. Sender has ${senderData.Okuse}, trying to send ${inputValue}.`);
//               throw new Error("Insufficient Okuse balance.");
//           }
...
//           setErrorMessage("Transfer failed. Please try again. " + error.message);
//       } else {
//           console.error("Transfer failed:", error);
//           setErrorMessage("Transfer failed. Please try again.");
//       }
...
import React, { useState, useEffect } from "react";
import { auth, db } from "../../../../firebase/firebase";
import { collection,getDoc, doc, getDocs, orderBy, query, runTransaction, setDoc, deleteDoc, writeBatch } from "firebase/firestore"; 
import { onAuthStateChanged } from "firebase/auth";
import "./transf.css";
...
const cleanUpOldTransactions = async (userId: string) => {
    try {
        ...
        if (snapshot.docs.length > 10) {
            ...
            console.log(`Deleting ${extraTransactions.length} old transactions for user ${userId}:`, extraTransactions);

            // Use a batch to delete all extra transactions at once
            const batch = writeBatch(db);

            extraTransactions.forEach((doc) => {
                batch.delete(doc.ref);
            });

            // Commit the batch
            await batch.commit();
            console.log(`Deleted ${extraTransactions.length} old transactions for user ${userId}.`);
        } else {
            console.log("No old transactions to delete.");
        }
    } catch (error) {
        console.error("Error cleaning up old transactions:", error);
        throw error; // Re-throw the error to handle it in ConfirmTransfer
    }
};
// const cleanUpOldTransactions = async (userId: string) => {
//   const userTransactionRef = collection(db, "users", userId, "Transaction_history");
